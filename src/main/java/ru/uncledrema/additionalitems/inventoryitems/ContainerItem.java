...
public class ContainerItem extends Container
{
	...
	final InventoryItemRoot inventory;
	 ...
	 * InventoryItemThree's number of slots (e.g. 5 slots is array indices 0-4, so start at 5)
	 * Notice how we don't have to remember how many slots we made? We can just use
	 * InventoryItemThree.INV_SIZE and if we ever change it, the Container updates automatically. */
	...
	// ARMOR_START = InventoryItemThree.INV_SIZE, ARMOR_END = ARMOR_START+3,
	// INV_START = ARMOR_END+1, and then carry on like above.

	public ContainerItem(EntityPlayer par1Player, InventoryPlayer inventoryPlayer, InventoryItemRoot inventoryItem)
	{
		INV_START = inventoryItem.INV_SIZE;
		...
		if (slot != null && slot.getHasStack())
		{
			...
			else
			{
				...
				if (itemstack1.getItem() instanceof ItemCustom)
				{
					...
					// We use 'InventoryItemThree.INV_SIZE' instead of INV_START just in case
					// you also add armor or other custom slots
					if (!this.mergeItemStack(itemstack1, 0, InventoryItemThree.INV_SIZE, false))
					{
						return null;
					}
				}
...
public class SlotItemInv extends Slot
{
	...
	public boolean isItemValid(ItemStack itemstack)
	{
		...
		return !(itemstack.getItem() instanceof ItemStoreThree || itemstack.getItem() instanceof ItemStoreTwo || itemstack.getItem() instanceof ItemStoreFive);
	}
	}
}