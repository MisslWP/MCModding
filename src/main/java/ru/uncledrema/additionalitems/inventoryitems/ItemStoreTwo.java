...
public class ItemStoreTwo extends Item
{
	private IIcon icon;
	public ItemStoreTwo()
	{
		...
		if (!world.isRemote)
		{
			...
			if (!player.isSneaking()) {
				...
				if (itemstack.hasTagCompound() && itemstack.getTagCompound().getString("playerBound").equals(player.getUniqueID().toString())) {
					player.openGui(InventoryItemMain.instance, InventoryItemMain.GUI_ITEM_INV_TWO, world, 0, 0, 0);
				}
    ...
    public void registerIcons(IIconRegister register) {
       this.icon = register.registerIcon("inventoryitemmod:bagepsilon");
    }
